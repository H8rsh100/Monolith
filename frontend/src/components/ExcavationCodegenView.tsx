...
interface ExcavationCodegenViewProps {
...
export const ExcavationCodegenView: React.FC<ExcavationCodegenViewProps> = ({
  programs,
  codegen,
  selectedProgram,
  onSelectProgram,
  ...
  return (
    <div className="w-full h-full p-6 topographic-strata text-slate-100 font-sans overflow-y-auto flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 border-b border-white/10 pb-4 text-xs font-mono">
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="bg-[#1e293b] border border-sky-500/40 text-sky-300 rounded-xl px-3.5 py-1.5 font-mono focus:outline-none"
          >
            {programs.map((p) => (
          ...
          <div className="flex items-center gap-1 bg-[#1e293b] p-1 rounded-xl border border-white/10">
            <button
              ...
              className={`px-3.5 py-1 rounded-lg font-bold transition-all ${
                targetLang === 'python' ? 'bg-sky-500 text-slate-950 shadow-md font-bold' : 'text-slate-400 hover:text-sky-300'
              }`}
            >
              PYTHON 3.12
            </button>
            <button
              ...
              className={`px-3.5 py-1 rounded-lg font-bold transition-all ${
                targetLang === 'java' ? 'bg-sky-500 text-slate-950 shadow-md font-bold' : 'text-slate-400 hover:text-sky-300'
              }`}
            >
              JAVA SPRING BOOT
            </button>
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-1.5 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold flex items-center gap-1.5 transition-all shadow-md shadow-sky-500/25"
        >
      ...
      {codegen ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
          ...
          <div className="dig-card flex flex-col overflow-hidden border-sky-500/30 font-mono">
            <div className="p-3 border-b border-white/10 bg-sky-500/10 flex items-center justify-between text-xs">
              <span className="font-bold text-sky-400">{codegen.stubFilename}</span>
              <button onClick={() => handleCopy(codegen.stubCode, true)} className="text-sky-300 hover:text-white flex items-center gap-1">
                {copiedStub ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
                {copiedStub ? 'COPIED!' : '[COPY STUB]'}
              </button>
            </div>
            <pre className="p-4 flex-1 overflow-y-auto bg-[#0a0f1d] text-sky-200 text-xs leading-relaxed whitespace-pre-wrap">
              {codegen.stubCode}
            </pre>
          </div>
          ...
          <div className="dig-card flex flex-col overflow-hidden border-sky-500/30 font-mono">
            <div className="p-3 border-b border-white/10 bg-amber-500/10 flex items-center justify-between text-xs">
              <span className="font-bold text-amber-300">{codegen.testFilename}</span>
              <button onClick={() => handleCopy(codegen.testCode, false)} className="text-amber-300 hover:text-white flex items-center gap-1">
                {copiedTest ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
                {copiedTest ? 'COPIED!' : '[COPY TESTS]'}
              </button>
            </div>
            <pre className="p-4 flex-1 overflow-y-auto bg-[#0a0f1d] text-amber-200/90 text-xs leading-relaxed whitespace-pre-wrap">
              {codegen.testCode}
            </pre>
          </div>
        ...
        <div className="dig-card p-12 text-center text-slate-400 font-mono">
