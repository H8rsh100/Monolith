...
// Senior FE Custom Node Component
const CustomNode: React.FC<NodeProps> = ({ data }) => {
  ...
  return (
    <div
      className="px-4 py-3 rounded-xl border bg-slate-900/90 text-white min-w-[190px] shadow-2xl backdrop-blur-md transition-all duration-300 hover:scale-105 group"
      style={{
...
const GraphContent: React.FC<GraphViewProps> = ({ graphData, onSelectProgram, onIngest, loading }) => {
  ...
  return (
    <div className="w-full h-[calc(100vh-4rem)] relative bg-slate-950 overflow-hidden">
      {nodes.length === 0 ? (
        ...
        >
          ...
          {/* Controls placed top-right to prevent bottom-left legend overlap */}
          <Controls className="!bg-slate-900/90 !border-slate-800 !text-slate-300 !rounded-xl !shadow-2xl !top-6 !right-6 !left-auto !bottom-auto" />
          
          <MiniMap
            nodeColor={(node) => (node.data?.riskColor as string) || '#38bdf8'}
            maskColor="rgba(9, 13, 22, 0.85)"
            className="!bg-slate-900/90 !border-slate-800/80 !rounded-xl !shadow-2xl !bottom-6 !right-20"
          />
        </ReactFlow>
      )}
