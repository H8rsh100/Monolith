...
interface StudioGraphViewProps {
...
const StudioNode: React.FC<NodeProps> = ({ data }) => {
  ...
  return (
    <div
      className="px-4 py-3 rounded-xl border bg-slate-900/90 text-white min-w-[200px] shadow-2xl backdrop-blur-md transition-all duration-300 hover:scale-105 group"
      style={{
    ...
    >
      ...
      {nodeType === 'program' && (
        <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-slate-800/80 text-xs">
          <span className="text-slate-400 font-mono text-[11px]">Risk Index:</span>
          <span
...
const GraphContent: React.FC<StudioGraphViewProps> = ({ graphData, onSelectProgram, onIngest, loading }) => {
  ...
  return (
    <div className="w-full h-full relative bg-[#080c14] overflow-hidden">
      {nodes.length === 0 ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#080c14] z-20">
      ...
      {nodes.length > 0 && (
        <div className="absolute top-6 left-6 z-10 flex items-center gap-3">
          <button
            ...
            className="px-3.5 py-2 rounded-xl studio-card text-xs font-mono font-semibold text-slate-300 hover:text-white hover:border-slate-700 transition-all flex items-center gap-2 shadow-xl"
          >
            <Maximize2 className="h-3.5 w-3.5 text-sky-400" />
            Recenter Graph
          </button>
          <div className="px-3 py-2 rounded-xl studio-card text-xs font-mono text-slate-400">
      ...
      <div className="absolute bottom-6 left-6 z-10 p-4 studio-card border border-slate-800/80 text-xs font-mono flex flex-col gap-2.5 shadow-2xl min-w-[200px]">
...
export const StudioGraphView: React.FC<StudioGraphViewProps> = (props) => (
  <ReactFlowProvider>
    <GraphContent {...props} />
  </ReactFlowProvider>
);
