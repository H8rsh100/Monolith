...
import { X, ShieldAlert, ArrowUpDown, Minimize2, Maximize2 } from 'lucide-react';
...
export const FloatingRiskTableHUD: React.FC<FloatingRiskTableHUDProps> = ({
  ...
  if (isMinimized) {
    return (
      <div className="fixed top-6 right-6 z-40 px-4 py-2 glass-hud flex items-center gap-3 border-cyanAccent/40 shadow-2xl font-mono text-xs text-cyanAccent">
        <ShieldAlert className="h-4 w-4 text-cyanAccent animate-pulse" />
        <span className="font-bold uppercase tracking-wider">Thermal Risk Matrix HUD (Collapsed)</span>
        <button
          onClick={() => setIsMinimized(false)}
          className="p-1 text-slate-400 hover:text-cyanAccent rounded"
          title="Expand Matrix HUD"
        >
  ...
  return (
    <div className="fixed top-6 right-6 bottom-6 z-40 w-[540px] glass-hud flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.8)] border-cyanAccent/30 overflow-hidden font-sans animate-in slide-in-from-right duration-300">
      ...
      <div className="p-3.5 border-b border-cyanAccent/20 bg-void/80 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-mono font-bold text-xs uppercase tracking-wider text-white">Thermal System Risk Matrix HUD</h3>
        </div>
